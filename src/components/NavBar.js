...
const NavBar = () => {
  ...
  return (
    <header className="w-full px-32 py-8 font-medium flex item-center justify-between dark:text-light relative z-10 lg:px-16 md:px-12 sm:px-8">
      ...
      >
        <span
          className={`bg-dark dark:bg-light transition-all ease-out duration-300 h-0.5 w-6 rounded-sm -translate-y-0.5 ${
            isOpen ? "rotate-45 translate-y-1" : "-translate-y-0.5"
          }`}
        ></span>
        <span
          className={`bg-dark dark:bg-light transition-all ease-out duration-300 h-0.5 w-6 rounded-sm my-0.5 ${
